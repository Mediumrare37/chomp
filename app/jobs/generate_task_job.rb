class GenerateTaskJob < ApplicationJob
  ...
  def openai_call_api(task)
    adapted_prompt = 'The user gives you a task, return an ruby hash with 5 key-value pairs. Each key (maxium 8 words) is a sub-task of the user prompt. Each value (maximum 8 words) is an array of 3 sub-tasks for the sub-task in the corresponding key. Follow the format in the example:
